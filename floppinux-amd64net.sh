...
dropbear_build () {
    ...
    printgr "DROPBEAR" "build the source code"
        toucher_dir "$TOUCH_TIME" "./"
    make CC="${MUSL_CC}" PROGRAMS="dbclient" dbclient scp "$THREAD_SETTINGS"
        toucher_dir "$TOUCH_TIME" "./"
...
' ./Makefile
        printgr "WPA_$1" "build the source code"
            toucher_dir "$TOUCH_TIME" "./"
        make CC="${MUSL_CC}" wpa_$1 "$THREAD_SETTINGS"
            toucher_dir "$TOUCH_TIME" "./"
...
LDFLAGS = '"$COMMON_LDFLAGS"' -static -no-pie' ./Makefile
    ...
    printgr "KIRC" "build the source code"
        toucher_dir "$TOUCH_TIME" "./"
    make CC="${MUSL_CC}" "$THREAD_SETTINGS"
        toucher_dir "$TOUCH_TIME" "./"
...
/^my \$platform = pop \@ARGV;/,/^\$cflags = "compiler: \$cflags";/ s/^/# /' ./util/mkbuildinf.pl
    printgr "OPENSSL" "configure the source code for toolchain directory installation"
    ...
    printgr "OPENSSL" "build the source code"
        toucher_dir "$TOUCH_TIME" "./"
    make CC="${MUSL_CC}" "$THREAD_SETTINGS" AR="${MUSL_AR}" RANLIB="${MUSL_RANLIB}"
        toucher_dir "$TOUCH_TIME" "./"
...
links_build () {
    ...
    # Set toolchain variables in the environment
    export CC="${MUSL_CC}"
    export AR="${MUSL_AR}"
    export RANLIB="${MUSL_RANLIB}"
    export CFLAGS="${COMMON_CFLAGS} -fno-pie --sysroot=${MUSLDIR} -I${MUSLDIR}/include -I${MUSLDIR}/usr/include"
    export LDFLAGS="${COMMON_LDFLAGS} -static -no-pie --sysroot=${MUSLDIR} -L${MUSLDIR}/lib -L${MUSLDIR}/x86_64-linux-musl/lib -L${MUSLDIR}/usr/lib64"
    ./configure \
...
__ /etc/get_repo.sh - downloads the kirc, openssl, links, etc. __
EOF
...
EOF
    ...
    ### NOTE: kirc does not fit into our floppy, so we will be downloading it with a ./get_repo.sh
